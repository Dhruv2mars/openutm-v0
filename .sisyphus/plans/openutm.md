...
- [ ] Implement data aggregation endpoint (or use PostHog/Plausible)
...
✅ **Metrics**: Opt-in telemetry (confirmed)
✅ **Framework**: Building BOTH Tauri and Electron for A/B testing

## Remaining Open Questions

1. **Telemetry Provider**: PostHog (free tier), Plausible (self-hosted), or custom endpoint?
2. **PostHog vs Plausible**: PostHog has generous free tier for open source; Plausible is privacy-focused but requires self-hosting
3. **Cloud VM Integration**: Future feature? (AWS, Azure, GCP)? Say no for now.
4. **Container Support**: Docker/Podman integration alongside VMs? Future phase.
5. **Mobile Version**: iOS/Android companion app? Not in roadmap.
